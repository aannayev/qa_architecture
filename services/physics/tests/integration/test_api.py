...
async def test_list_questions_does_not_leak_correct_answer(client, seeded):
    response = await client.get("/v1/questions?limit=10")
    assert response.status_code == 200
    data = response.json()
    assert len(data) > 0
    for item in data:
...
async def test_list_questions_filter_by_topic(client, seeded):
    response = await client.get("/v1/questions?topic=mechanics")
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 2
    assert all(q["topic"] == "mechanics" for q in data)
...
async def test_get_question_by_id(client, seeded):
    list_resp = await client.get("/v1/questions?topic=electromagnetism&limit=1")
...
async def test_submit_correct_answer(client, seeded):
    list_resp = await client.get("/v1/questions?topic=electromagnetism")
    question = next(q for q in list_resp.json() if q["external_id"] == "phys-em-001")
    ...
    assert "299,792,458" in body["explanation"] or "3 x 10^8" in body["explanation"]
...
async def test_list_topics(client, seeded):
    ...
    assert topics == {"mechanics", "thermodynamics", "electromagnetism", "modern-physics"}
