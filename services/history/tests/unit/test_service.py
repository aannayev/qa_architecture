...
def populated_service() -> tuple[QuestionService, list[Question]]:
    questions = [
        make_question(topic="ancient-world", difficulty=Difficulty.EASY),
        make_question(topic="ancient-world", difficulty=Difficulty.HARD),
        make_question(topic="modern", difficulty=Difficulty.MEDIUM),
    ]
...
async def test_list_filters_by_topic(populated_service):
    ...
    results = await svc.list_questions(topic="ancient-world", difficulty=None, limit=20)
    assert len(results) == 2
    assert all(q.topic == "ancient-world" for q in results)
...
async def test_topics_counts(populated_service):
    ...
    as_dict = dict(topics)
    assert as_dict == {"ancient-world": 2, "modern": 1}
